...
public class SNMPEvent implements IDataProcessChain{
	...
	public Object dataProcess(Object rawData) throws Exception {
		...
		Map<String,String> snmpResult=(Map<String, String>) snmpData.get(MetaDataConstant.EXECUTE_RESULT);
		...
		for(String resultKpiID:snmpResult.keySet()){
			...
			if(TIMEOUT.equals(value)){
				if(timeoutFlag==0){
					EventAndAlarm.JudgeGenerateAndRecoveryEvent(resID, TIMEOUT, TIMEOUT);
					timeoutFlag=1;
				}
				break;
			}
			if(NOSUCHOBJECT.equals(value)){
				if(nosuchobjceFlag==0){
					EventAndAlarm.JudgeGenerateAndRecoveryEvent(resID, NOSUCHOBJECT, NOSUCHOBJECT);
					nosuchobjceFlag=1;
				}
				continue;
			}
		...
		if(timeoutFlag==0){
			EventAndAlarm.isRecoveryEvent(resID, TIMEOUT);
		}
		if(nosuchobjceFlag==0){
			EventAndAlarm.isRecoveryEvent(resID, NOSUCHOBJECT);
		}
		return snmpData;
	}
