...
public class SyslogStandard implements IDataProcessChain {
	...
	public Object dataProcess(Object rawData) throws Exception {
		...
		if (syslogRuleService == null) {
			theLogger.error(
					"syslog standard need the SyslogStandardRuleService.");
			return null;
		}
		Map<String,Map<String,Map<String,String>>> ruleMapping=syslogRuleService.get_ruleMapping();
		if ( ruleMapping== null||ruleMapping.size()==0) {
			theLogger.debug("rule mapping is null,don't need standard the syslog data");
			return rawData;
		}
		...
		for (Map<String, Object> syslogData : syslogDatas) {
			...
			Map<String,Map<String,String>> ruleNameMapping=ruleMapping.get(typeCode);
			if(ruleNameMapping==null||ruleNameMapping.size()==0){
				continue;
			}
			syslogData.put(MetaDataConstant.EXECUTE_RESULT, syslogStandard(ruleNameMapping,result));
		...
		for(String key:resultMapping.keySet())
		{
			...
			if(DATE_PROPERTY_NAME.equals(key)){
				...
				if(!Assert.isEmptyString(dateRule)){
					resultMapping.put(key, dateStandard(resultMapping.get(key),dateRule));
		...
		try {
			...
			String dateAfter = null;
				...
				if (dateRule.indexOf("y") == -1) {
					...
					SimpleDateFormat format = new SimpleDateFormat(dateRule, Locale.US);
					
						Date dateD = format.parse(dateBefore);
						SimpleDateFormat simpeleDataFormat = new SimpleDateFormat("MMddHHmmss");
					...
					SimpleDateFormat format = new SimpleDateFormat(dateRule, Locale.US);
						...
						Date dateD = format.parse(dateBefore);
						SimpleDateFormat simpeleDataFormat = new SimpleDateFormat("yyyyMMddHHmmss");
