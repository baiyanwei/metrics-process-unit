...
public class SSHTelnetDBStorageAdapter extends DBStorage {
	private static PlatformLogger theLogger = PlatformLogger
			.getLogger(SSHTelnetDBStorageAdapter.class);
	...
	public void dbStorage(Object storeData) throws Exception {
		...
		if (storeData == null) {
			throw new PlatformException(
					"invalid store data in ssh or telnet database storage.");
		}
		...
		String[] executeResult = (String[]) sshOrTelnetData
		...
		if (Assert.isEmptyString(executeResult[1])) {
			...
			configAndPolicyInfo.append("#policy#" + executeResult[1]
					+ "#policy#");
		}
		...
		configAndPolicyBean.setConfigAndPolicyInfo(configAndPolicyInfo
				.toString());
