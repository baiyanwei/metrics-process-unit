...
public class PolicyContainAndConflict {
	private static PlatformLogger theLogger = PlatformLogger.getLogger(PolicyContainAndConflict.class);
	...
	public String[] policyContainAndConflict(){
		if(Assert.isEmptyString(policyValues)||Assert.isEmptyString(containAndConflictRules)){
			return null;
		}
	...
	private String allMatch(String rule, String[] values) {
		...
		for(int i=0;i<values.length;i++){
			...
			for(int j=0;j<values.length;j++){
				...
				int matchResultFlag=PolicyMatchUtil.matchTwoValue(values[i],values[j],rule);
				if(matchResultFlag==1){
					String valueAfter=PolicyMatchUtil.getNameValue(values[j],StandardConstant.ORIGIN_NAME);
					if(Assert.isEmptyString(valueAfter)){
						continue;
					}
					if(valueBeforeFlag==0){
						String valueBefore=PolicyMatchUtil.getNameValue(values[i],StandardConstant.ORIGIN_NAME);
