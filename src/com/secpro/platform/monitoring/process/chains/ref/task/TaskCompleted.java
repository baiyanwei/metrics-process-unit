...
public class TaskCompleted extends Thread{
	...
	public void run(){
		...
		if (MetaDataParsing.isCacheTask(taskCode)) {
			...
			taskBean.setExecuteAt(Long.parseLong((String) _data.get(MetaDataConstant.EXECUTE_DATE)));
			taskBean.setExecuteCost(Long.parseLong((String) _data.get(MetaDataConstant.EXECUTE_COST)));
			taskBean.setExecuteStatus(_executeResult);
			if(TASK_SUCCESS==_executeResult){
				taskBean.setExecuteDes(TASK_SUCCESS_DES);
			}else{
				taskBean.setExecuteDes((String) _data.get(MetaDataConstant.ERROR_DESCRIPTION));
			}
